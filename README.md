...
Phase 4 starts with value-free intake profiling and an active reference steward agent. The agent is a separate model-independent supervisor that advances through `PROFILE`, `IDENTIFY`, `PLAN`, and `VERIFY` under fixed per-phase MCP allowlists and a 16-call ceiling. It can create and simulate a proposal but cannot approve or execute it. Its production transport performs authenticated Streamable HTTP MCP calls and retains decision codes plus tool outcomes without hidden reasoning or credentials.

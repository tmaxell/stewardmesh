...
# Vendor and assistant names are forbidden anywhere. Attribution keywords are forbidden only where
# they actually attribute, that is at the start of a trailer or footer line, so documentation and
# commit messages may still name the rule they enforce.
PROVENANCE_VENDOR='claude|anthropic|copilot|chatgpt|openai|gemini'
...
PROVENANCE_FILE_HITS="$(
  git grep -InE -i "${PROVENANCE_VENDOR}" -- ${PROVENANCE_SCANNED} || true
...
PROVENANCE_COMMIT_HITS="$(
  grep -inE "${PROVENANCE_VENDOR}" <<<"${PROVENANCE_HISTORY}" || true
